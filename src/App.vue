<template>
  <div id="app">
    ...
    <Tables :dataItem="dataList"></Tables>
    <el-button :plain="true" @click="open8" v-show="isBtn" :style="{opacity:0}">错误</el-button>
  </div>
</template>
  ...
  export default {
    ...
    mounted() {
      for(var attr in this.states){
        this.list.push({value:attr,label:attr,english:this.states[attr]});
      }
    },
    methods: {
      open8() {
      ...
      handleIconClick(e){
        ...
        this.$http.get('/list',{
          params:{
            type:this.changeArr[0].english,
            postid:Number(this.input2)
          }
        }).then( (data)=> {
          console.log(90000002506293)
          console.log(70972420417304)
          console.log(117081584613238401)
      ...
      change(val){
        this.changeArr = this.options4.filter(function (item) {
          if(item.value ==val){
            return true
          }
          return false;
